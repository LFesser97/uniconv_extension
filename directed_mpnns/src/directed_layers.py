...
"""
def construct_Pi_A(edge_index: torch.Tensor, edge_weight: torch.Tensor = None,
                     num_nodes: int = None, dtype: torch.dtype = torch.float32):
    if num_nodes is None:
        num_nodes = int(edge_index.max().item()) + 1
    if edge_weight is None:
        edge_weight = torch.ones(edge_index.size(1), dtype=dtype, device=edge_index.device)

    # Create a sparse tensor for A and convert it to a dense matrix.
    A = torch.sparse_coo_tensor(edge_index, edge_weight, size=(num_nodes, num_nodes),
                                  dtype=dtype, device=edge_index.device)
    A_dense = A.to_dense()

    # Compute symmetric and antisymmetric parts.
    sym = A_dense + A_dense.t()      # A + A^T
    antisym = A_dense - A_dense.t()  # A - A^T

    # Compute the skew-Hermitian projection:
    # Pi(A) = (antisym - i * sym) / 4
    Pi = (antisym + 1j * sym) / 4.0

    # Convert Pi back to a sparse representation.
    Pi_nonzero = Pi.nonzero(as_tuple=False)
    Pi_edge_index = Pi_nonzero.t().contiguous()  # shape: [2, E_new]
    Pi_edge_weight = Pi[Pi_nonzero[:, 0], Pi_nonzero[:, 1]]

    return Pi_edge_index, Pi_edge_weight
"""
...
def construct_Pi_A(edge_index: torch.Tensor, edge_weight: torch.Tensor = None,
    ...
    A = torch.sparse_coo_tensor(edge_index, edge_weight,
                                ...
                                dtype=complex_dtype, device=edge_index.device).coalesce()
    A_T = torch.sparse_coo_tensor(edge_index[[1, 0]], edge_weight,
                                  size=(num_nodes, num_nodes),
                                  dtype=complex_dtype, device=edge_index.device).coalesce()
    sym = A + A_T
    antisym = A - A_T
...
class DirectedUnitaryGCNConvLayer(nn.Module):
    def __init__(self, dim_in, dim_out, dropout, residual, global_bias = False, **kwargs):
        super().__init__()
        self.dim_in = dim_in
        ...
        if global_bias:
            # store bias as paired real-valued parameters (FSDP-safe)
            self.bias_real = torch.nn.Parameter(torch.zeros(dim_out))
            self.bias_imag = torch.nn.Parameter(torch.zeros(dim_out))
        else:
    ...
    def forward(self, x, edge_index):
        ...
        if x.requires_grad:
            x = checkpoint(lambda t: self.model(t, edge_index), x)
        else:
            x = self.model(x, edge_index)
        if getattr(self, 'bias_real', None) is not None and getattr(self, 'bias_imag', None) is not None:
            x = x + torch.complex(self.bias_real, self.bias_imag)
        x = self.act(x)
...
class ComplexGCNConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Two real-valued linear layers represent the real/imag parts of a complex weight matrix
        self.lin_real = torch.nn.Linear(in_channels, out_channels, bias=False)
        self.lin_imag = torch.nn.Linear(in_channels, out_channels, bias=False)
        self.lin_real.weight.data = init.orthogonal_(self.lin_real.weight.data)
        self.lin_imag.weight.data.zero_()

        if bias:
            self.bias_real = torch.nn.Parameter(torch.zeros(out_channels))
            self.bias_imag = torch.nn.Parameter(torch.zeros(out_channels))
        else:
            self.register_parameter('bias_real', None)
            self.register_parameter('bias_imag', None)
    ...
    def reset_parameters(self):
        ...
        self.lin_real.reset_parameters()
        self.lin_imag.reset_parameters()
        self._cached_edge_index = None
        self._cached_adj_t = None
        if getattr(self, 'bias_real', None) is not None:
            zeros(self.bias_real)
        if getattr(self, 'bias_imag', None) is not None:
            zeros(self.bias_imag)
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        if apply_feature_lin:
            ...
            # complex linear: (a+ib)W = (aW_r - bW_i) + i(aW_i + bW_r)
            a = x.real
            b = x.imag
            real_out = self.lin_real(a) - self.lin_imag(b)
            imag_out = self.lin_real(b) + self.lin_imag(a)
...
class TaylorGCNConv(MessagePassing):
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_weight: OptTensor = None) -> Tensor:
        ...
        c = 1.
        # initial feature transform (not checkpointed if input has no grad)
        ...
        for k in range(self.T):
            if x_k.requires_grad:
                x_k = checkpoint(lambda t: self.conv(t, edge_index, edge_weight, apply_feature_lin=False), x_k) / (k+1)
            else:
...
def gcn_norm(  # noqa: F811
    ...
    deg_inv_sqrt = (deg+1).pow_(-0.5)
    deg_inv_sqrt.masked_fill_(deg_inv_sqrt == float('inf'), 0)
    edge_weight = deg_inv_sqrt[row] * edge_weight * deg_inv_sqrt[col]

    edge_index, edge_weight = construct_Pi_A(edge_index=edge_index, edge_weight=edge_weight)
