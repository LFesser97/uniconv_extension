...
class DirectedUnitaryGCNConvLayer(nn.Module):
    def __init__(self, dim_in, dim_out, dropout, residual, global_bias = True, **kwargs):
        ...
        self.model = TaylorGCNConv(base_conv(dim_in, dim_out, **kwargs), T=16)
...
@register_layer('unitarygcnconv_enhanced')
class EDUnitaryGCNConvLayer(nn.Module):
    def __init__(self, dim_in, dim_out, dropout = 0.1, residual = True, 
                 global_bias=False, **kwargs):
        super().__init__()
        self.dim_in = dim_in
        self.dim_out = dim_out
        self.dropout = dropout
        self.residual = residual
        self.edge_feature_map = nn.Linear(dim_in, 1)
        if global_bias:
            self.bias = nn.Parameter(torch.zeros(dim_out, dtype=torch.cfloat))
        else:
            self.register_parameter('bias', None)
        base_conv = ComplexGCNConv(dim_in, dim_out, bias=False, **kwargs)
        # base_conv = ComplexGCNConv(dim_in, dim_out, cached=True, bias=False, **kwargs)
        self.model = TaylorGCNConv(base_conv)
        self.act = nn.Sequential(
            register.act_dict["c_gelu"](),
            ComplexDropout(self.dropout),
        )
    ...
    def forward(self, batch):
        ...
        if batch.edge_attr is None:
            edge_weight = torch.ones(batch.edge_index.size(1), dtype=batch.x.dtype, device=batch.x.device)
        else:
            edge_weight = self.edge_feature_map(batch.edge_attr)
            edge_weight = F.sigmoid( edge_weight.squeeze(-1) )
        batch.x = self.model(batch.x, batch.edge_index, edge_weight)
        if self.bias is not None:
            batch.x = batch.x + self.bias
        # batch.x = self.act(batch.x)
        batch.x = self.act(batch.x).real
...
class ComplexGCNConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._cached_num_nodes  = None   # ← new
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        # -- cache‐invalidation for inductive batches --
        if self.cached:
            # how many nodes in this batch?
            N = x.size(self.node_dim)
            # if first time, or shape changed, clear old cache
            if self._cached_num_nodes is None or self._cached_num_nodes != N:
                self._cached_edge_index = None
                self._cached_adj_t      = None
                self._cached_num_nodes  = N
